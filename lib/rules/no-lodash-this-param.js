	...
	return {
		VariableDeclarator: function( node ) {
			if ( node.init &&
				...
				node.init.callee.arguments &&
				node.init.callee.arguments.length === 1 &&
				~node.init.callee.arguments[0].value.indexOf( 'lodash/' ) ) {
				const lodashFunction = node.init.callee.arguments[0].value.split( '/' )[1];
